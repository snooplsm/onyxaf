...
import us.wmwm.onyx.db.OnyxSettings
import us.wmwm.onyx.db.Temp
import us.wmwm.onyx.domain.FlashData
import us.wmwm.onyx.domain.MonitorData
...
class PulseViewModel(val bt: BluetoothManager, val db: OnyxDb) : ViewModel() {
    ...
    fun onTemp(temp: Temp) {
        Single.just(1)
            ...
            .observeOn(Schedulers.io())
            .subscribe { t1, t2 ->
                settings = settings.copy(temp = temp)
                temps.postValue(temp)
            }
    }
